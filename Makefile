...
.PHONY: build test tv-test release run install clean
...
tv-test:
	env $(BUILD_ENV) LGVOLUME_RUN_INTEGRATION_TESTS=1 swift test --filter WebOSIntegrationTests

release:
	env $(BUILD_ENV) ./Scripts/release.sh
